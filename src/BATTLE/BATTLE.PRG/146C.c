...
int func_80081020(int, _hitEntity_t*);
...
extern short (*_damageCalculators[])(vs_skill_t*, _hitEntity_t*, _hitEntity_t*, int, int);
...
void func_80069D78(void)
{
    ...
    _camera.firstPersonViewFacing =
        ((_camera.firstPersonViewFacing & 0x7FFF) | (_firstPersonViewEnabled << 0x10));
}
    ...
    vs_battle_uiShield* target, vs_battle_shieldIntermediate* source)
{
    ...
    target->wepId = source->base.wepId;
    target->dropRate = source->dropRate;
    target->index = source->index;
...
void _applyAccessoryStats(vs_battle_uiAccessory* target, _accessoryIntermediate* source)
{
    int i;
    ...
    vs_battle_inventoryArmor* target, vs_battle_uiArmor* source)
{
    if (source->armor.id != 0) {
        ...
        vs_battle_copyUiEquipmentStats(target, &source->armor);
    }
}
...
void _enemyDefeated(int arg0, int actorId, int arg2)
{
...
void _clampPositionToZoneBounds(VECTOR* inPos, VECTOR* outPos)
{
    ...
    if (maxBound < bound) {
        outPos->vx = (bound + maxBound) >> 1;
    } else {
        if (inPos->vx < bound) {
            outPos->vx = bound;
        } else if (maxBound < inPos->vx) {
            outPos->vx = maxBound;
        } else {
            outPos->vx = inPos->vx;
        }
    ...
    bound = _zoneContext.bounds.minZ * ONE * 128 + entityRadius * 360;
    maxBound = _zoneContext.bounds.maxZ * ONE * 128 - entityRadius * 360;

    if (maxBound < bound) {
        outPos->vz = (bound + maxBound) >> 1;
    } else {
        if (inPos->vz < bound) {
            outPos->vz = bound;
        } else if (maxBound < inPos->vz) {
            outPos->vz = maxBound;
        } else {
            outPos->vz = inPos->vz;
        }
...
int _computeStepAndDisplacement(SVECTOR* targetPos, VECTOR* outStep,
    VECTOR* outDisplacement, int clampToZone, int maxSpeed)
{
    SVECTOR targetPosScaled;
    VECTOR targetPosFixed;
    VECTOR direction;
    long sxy;
    long p;
    long notCentered;

    targetPosFixed.vx = targetPos->vx * ONE;
    targetPosFixed.vz = targetPos->vz * ONE;
    targetPosFixed.vy = targetPos->vy * ONE;

    if (clampToZone != 0) {
        _clampPositionToZoneBounds(&targetPosFixed, &targetPosFixed);
    }

    targetPosScaled.vx = targetPosFixed.vx / ONE;
    targetPosScaled.vz = targetPosFixed.vz / ONE;
    targetPosScaled.vy = targetPosFixed.vy / ONE;

    SetRotMatrix(&_camera.viewMatrix);
    SetTransMatrix(&_camera.viewMatrix);
    RotTransPers(&targetPosScaled, &sxy, &p, &notCentered);

    if (clampToZone == 0) {
        int temp_a0 = sxy / 0x10000;
        if (((sxy - (temp_a0 * 0x10000)) - 61) < 199U) {
            int new_var = 180;
            if ((temp_a0 > 60) && (temp_a0 < new_var)) {
                notCentered = 0;
            } else {
                notCentered = 1;
            }
    ...
    if (notCentered) {
        ...
        if (direction.vx != 0) {
            distanceToTarget = outDisplacement->vx / direction.vx;
        } else if (direction.vz != 0) {
            distanceToTarget = outDisplacement->vz / direction.vz;
        } else if (direction.vy != 0) {
            distanceToTarget = outDisplacement->vy / direction.vy;
        } else {
            distanceToTarget = 1;
        }
...
void func_8006E640(int arg0)
{
    ...
    case 3:
        ...
        if ((D_800F19CC->unk298D != 0xFF) && (D_800F19CC->unk2C00 != 0)) {
            targetPos.vx = D_800F19CC->unk29C0[D_800F19CC->unk298D].unk0.vx;
            targetPos.vy = D_800F19CC->unk29C0[D_800F19CC->unk298D].unk0.vy;
            targetPos.vz = D_800F19CC->unk29C0[D_800F19CC->unk298D].unk0.vz;
        }
        _stepTowardTarget(&targetPos, 0, 0x28);
        break;
    case 2:
        func_8006E158();
        targetPos.vx = D_800F19CC->unk2998.vx;
        targetPos.vy = D_800F19CC->unk2998.vy;
        targetPos.vz = D_800F19CC->unk2998.vz;
        if (func_800C4734() == 1) {
            _stepTowardTarget(&targetPos, 0, 8);
            return;
        }
        _stepTowardTarget(&targetPos, 0, 0x28);
        break;
    case 11:
        func_8006E158();
        _stepTowardTarget(&targetPos, 1, 0x28);
        break;
    case 4:
        func_8006E158();
        break;
...
void func_8007647C(int arg0, int arg1)
{
    ...
    vs_main_bzero(temp_s0, sizeof *temp_s0);
    ...
    int arg0, int actorId, func_8007C8F8_t_flags* arg2, int material)
{
    ...
    if (vs_battle_actors[arg0] == NULL) {
        ...
        temp_v0->unk0.unk0.unk44 = &temp_v0->unk9B4;
    ...
    int arg0, vs_battle_actor2* arg1, vs_battle_charInitData* arg2, int arg3)
{
    ...
    arg1->reach = arg2->reach;
    arg1->unk956_2 = arg2->unk23;
    ...
    arg1->currentRange = arg1->weapon.range.range + arg1->reach;
    arg1->unk39 = arg1->weapon.range.unk1 + arg1->reach;
    arg1->unk3A = arg1->weapon.range.unk2 + arg1->reach;
    arg1->currentAttackShapeAngle = arg1->weapon.range.attackShapeAngle;
    arg1->unk3B_0 = 1;
    ...
    for (i = 0; i < 6; ++i) {
        ...
        arg1->bodyParts[i].agilityDefenseBonus = arg2->unk238[i].agilityDefenseBonus;
        ...
        arg1->bodyParts[i].chainEvasion = arg2->unk238[i].chainEvasion;
...
{
    vs_battle_actor* temp_s1 = vs_battle_actors[id];
    vs_battle_actor2* temp_s0 = temp_s1->unk3C;
    ...
    temp_s0->weapon.range.range = 5;
    temp_s0->weapon.range.unk1 = 4;
    temp_s0->weapon.range.unk2 = 5;
    temp_s0->unk956_0 = 0;
    temp_s0->weapon.range.attackShapeAngle = 0;
    temp_s1->unk27 = 128;
    temp_s1->unk29 = 0;
    temp_s1->defeated = 0;
    temp_s0->unk957 = 128;
    ...
    int shieldWepId, int shieldMaterial, func_8007C8F8_t_flags* arg6, int arg7)
{
    ...
    if (arg0 < 0xA) {
        temp_v0 = vs_main_allocHeap(sizeof *temp_v0);
        vs_main_bzero(temp_v0, sizeof temp_v0->unk0.unk0);
        i = 0xA;
        temp_v0->unk0.unk0.unk0.unk3C = &temp_v0->unk0.unk0.unk50;
        temp_v0->unk0.unk0.unk0.unk40 = 1;
        temp_v0->unk0.unk0.unk0.unk44 = &temp_v0->unk0.unk9B4;
...
int _isLookAtAtDestination(void)
{
    ...
    if ((sp10.position.vx == (sp10.lookAt.vx & ~0xFFF))
        ...
        && (sp10.position.vy == (sp10.lookAt.vy & ~0xFFF))) {
        return 1;
    }
    return 0;
}
...
void func_8007BA98(int id0, int count0, int id1, int count1)
{
    ...
    if (temp_v0 != NULL) {
        ...
        if (temp_v0->actorId != 0) {
            temp_a0 = _lootListHead;
            temp_v0->actorId = 0xFF;
            _lootListHead = temp_v0;
...
{
    func_8008C40C();
    return _firstPersonViewEnabled = ~arg0 & 1;
}
...
int func_8007C928(u_int arg0, int arg1, func_8007C8F8_t_flags* arg2)
{
    ...
    if (arg0 < 16) {
        if (vs_battle_actors[arg0] == NULL) {
            ...
            temp_v0 = vs_main_allocHeap(sizeof *temp_v0);
            *temp_s1 = &temp_v0->unk0.unk0.unk0;
            temp_v0->unk0.unk0.unk0.unk3C = &temp_v0->unk0.unk0.unk50;
            temp_v0->unk0.unk0.unk0.unk40 = 1;
            temp_v0->unk0.unk0.unk0.unk44 = &temp_v0->unk0.unk9B4;
            temp_v0->unk0.unk0.unk0.unk48[0] = &temp_v0->unk22B4[0];
    ...
    if (temp_s1 != NULL) {
        ...
        if (temp_s0 != NULL) {
            ...
            if (temp_s0->shield.base.id != 0) {
                func_80077130(temp_s1, arg0, temp_s0->shield.wepId, 1,
...
short _getAttackGemBuff(vs_skill_t* skill, vs_battle_actor2* actor)
{
    ...
    if (vs_battle_actors[actor->unk957]->unk20 & 1) {
        ...
        if (skill->type == skillTypeSpell) {
            ...
            hasGemBuff = 0;
            for (i = 0; i < 3; ++i) {
                if (actor->weapon.gems[i].gemEffects == 1) {
                    hasGemBuff = 1;
                }
            }
        }
    ...
    if (vs_battle_actors[actor->unk957]->unk20 & 1) {
        ...
        if (skill->type == 1) {
            ...
            if ((v < 40) || (((v - 54) & 0xFF) < 170)) {
                hasGemBuff = 0;
                for (i = 0; i < 3; ++i) {
                    if (actor->shield.gems[i].gemEffects == 5) {
                        hasGemBuff = 1;
                    }
                }
                if (hasGemBuff != 0) {
        ...
        case 2:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 12) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 3:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 13) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 5:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 14) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 11:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 16) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 15:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 18) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 16:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 19) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
        ...
        case 17:
            for (i = 0; i < 3; ++i) {
                if (actor->shield.gems[i].gemEffects == 21) {
                    hasGemBuff = 1;
                }
            }
            if (hasGemBuff != 0) {
    ...
    int hitNumber, int arg4 __attribute__((unused)))
{
    ...
    if ((source != NULL) && (source->unk40 == 0)) {
        gemModifier += _getAttackGemBuff(skill, vs_battle_actors[source->actorId]->unk3C);
    }
    if ((target != NULL) && (target->unk40 == 0)) {
        gemModifier -= _getDefenseGemBuff(
            skill, vs_battle_actors[target->actorId]->unk3C, hitNumber);
    }
    return gemModifier;
}
    ...
    int hitNumber, int addVariance)
{
    int agiDiff;
    int i;
    int targetBodyPart = target->targetedBodyPart;

    vs_battle_actor2* sourceActor = vs_battle_actors[source->actorId]->unk3C;
    vs_battle_actor2* targetActor = vs_battle_actors[target->actorId]->unk3C;
    int sourceActorValue = sourceActor->agility + sourceActor->accessory.currentAgility;
    ...
    _hitEntity_t* target, int hitNumber, int addVariance)
{
    ...
    vs_battle_actor2* sourceActor = vs_battle_actors[source->actorId]->unk3C;
    vs_battle_actor2* targetActor = vs_battle_actors[target->actorId]->unk3C;
    int sourceActorValue = sourceActor->intelligence + sourceActor->accessory.currentInt;
    ...
    int hitNumber, int addVariance)
{
    ...
    vs_battle_actor2* sourceActor = vs_battle_actors[source->actorId]->unk3C;
    vs_battle_actor2* targetActor = vs_battle_actors[target->actorId]->unk3C;
    int sourceValue = sourceActor->agility + sourceActor->accessory.currentAgility;
...
int _getRiskModifier(vs_skill_t* arg0 __attribute__((unused)), _hitEntity_t* source,
    ...
    int arg4 __attribute__((unused)))
{
    int rate = ((vs_battle_actors[source->actorId]->unk3C->risk + 150) * 100) / 256;
    if (rate == 255) {
        rate = 254;
    }
    return rate;
    ...
    int hitNumber, int addVariance)
{
    ...
    if ((target->unk40 == 0)
        && (vs_battle_actors[target->actorId]->unk3C->unk957 == 0x80)) {
        threshold = 100;
    } else {
    ...
    if ((hitNumber == 0) || (target->hitThreshold == 255)) {
        target->hitThreshold = threshold;
    }

    chance = vs_main_getRand(100);
...
void func_80080000(vs_skill_t* skill, _hitEntity_t* entity, short baseValue)
{
    ...
    for (i = 0; i < 6; ++i) {
        _armorDpAdjustmentAmounts[i] = 0;
    }

    actor = vs_battle_actors[entity->actorId]->unk3C;
    ...
    case 4:
        ...
        for (i = 0; i < 6; ++i) {
            if (actor->bodyParts[i].unk2 != 0) {
                _armorDpAdjustmentAmounts[i] = baseValue / var_t0;
    ...
    case 14:
        for (i = 0; i < 6; ++i) {
            _armorDpAdjustmentAmounts[i] =
                (baseValue * actor->unk920[entity->targetedBodyPart][i]) / 100;
            entity->unk20.s16[i][0] += _armorDpAdjustmentAmounts[i];
        }
        break;
    case 6:
    ...
    case 11:
    case 12:
        _armorDpAdjustmentAmounts[entity->targetedBodyPart] = baseValue;
        entity->unk20.s16[entity->targetedBodyPart][0] += baseValue;
        break;
    }
}

void func_800801E0(vs_skill_t* skill, _hitEntity_t* entity, short arg2)
{
    int temp_lo;
    int i;
    int var_t0;
    ...
    for (i = 0; i < 6; ++i) {
        if (actor->bodyParts[i].unk2 != 0) {
            var_t0 += actor->bodyParts[i].unk2 - actor->bodyParts[i].unk0;
        }
    }
    ...
    if (var_t0 != 0) {
        for (i = 0; i < 6; ++i) {
            if (actor->bodyParts[i].unk2 != 0) {
                temp_lo = (arg2 * (actor->bodyParts[i].unk2 - actor->bodyParts[i].unk0))
                        / var_t0;
    ...
    vs_skill_t* skill, vs_battle_actor2* source, vs_battle_actor2* target, int amount)
{
    ...
    if (skill->type != skillTypeSpell) {
        if (source->weapon.currentDp >= amount) {
            ...
            source->weapon.currentDp = 0;
        }
    }

    if (vs_battle_actors[target->unk957]->unk20 & 1) {
        if (target->shield.currentPp >= amount) {
            target->shield.currentPp -= amount;
        } else {
            target->shield.currentPp = 0;
        }
    }
...
int func_80081020(int arg0, _hitEntity_t* arg1)
{
    int temp_t0;
    int var_a3;
    int var_t1;
...
short func_800821B0(vs_skill_t* arg0, _hitEntity_t* arg1,
    ...
    int arg4 __attribute__((unused)))
{
    vs_battle_actor2* temp_v0 = vs_battle_actors[arg1->actorId]->unk3C;
...
short func_800822A0(vs_skill_t* arg0, _hitEntity_t* arg1 __attribute__((unused)),
    ...
    int arg4 __attribute__((unused)))
{
    int v = vs_battle_actors[arg2->actorId]->unk3C->currentHP
          * arg0->hitParams[arg3].damageFactor;
...
short func_8008231C(vs_skill_t* arg0 __attribute__((unused)), _hitEntity_t* arg1,
    ...
    int arg4 __attribute__((unused)))
{
    return vs_battle_actors[arg1->actorId]->unk3C->weapon.currentPp
...
short func_800834A4(vs_skill_t* arg0 __attribute__((unused)),
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    return vs_battle_actors[arg2->actorId]->unk3C->maxHP
         - vs_battle_actors[arg2->actorId]->unk3C->currentHP;
}
...
short func_800834E4(vs_skill_t* arg0 __attribute__((unused)),
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    return vs_battle_actors[arg2->actorId]->unk3C->maxMP
         - vs_battle_actors[arg2->actorId]->unk3C->currentMP;
}
...
short vs_battle_getEquippedShieldDp(vs_skill_t* arg0 __attribute__((unused)),
    _hitEntity_t* arg1, _hitEntity_t* arg2 __attribute__((unused)),
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    return vs_battle_actors[arg1->actorId]->unk3C->shield.currentDp;
}
...
short _getRisk(vs_skill_t* arg0 __attribute__((unused)),
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    return vs_battle_actors[arg2->actorId]->unk3C->risk;
}
    ...
    vs_skill_t* skill, _hitEntity_t* arg1, _hitEntity_t* arg2, int hit, int arg4)
{
    ...
    if ((arg2->unk40 == 0) && (vs_battle_actors[arg2->actorId]->unk3C->unk957 == 0x80)) {
        damage = 999;
    } else {
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    ...
    arg2->unk4 = vs_battle_actors[arg2->actorId]->unk3C->currentHP;
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    ...
    arg2->unk20.s16[arg2->targetedBodyPart][1] = arg2->unk4 * 2;
...
{
    short temp_v0;
    vs_battle_actor2* temp_s4 = vs_battle_actors[arg1->actorId]->unk3C;
    vs_battle_actor2* temp_s2 = vs_battle_actors[arg2->actorId]->unk3C;
    D_800F1A0C = 0;
...
int func_80084CAC(
    vs_skill_t* arg0, _hitEntity_t* arg1, _hitEntity_t* arg2, int arg3, int arg4)
{
    vs_battle_actor2* temp_s0 = vs_battle_actors[arg2->actorId]->unk3C;
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    ...
    if (vs_battle_actors[arg2->actorId]->unk3C->weapon.blade.id != 0) {
        new_var = vs_battle_actors[arg2->actorId]->unk3C->weapon.currentPp;
...
int func_80084DA8(vs_skill_t* arg0 __attribute__((unused)),
    ...
    int arg3 __attribute__((unused)), int arg4 __attribute__((unused)))
{
    vs_battle_actor2* temp_a1 = vs_battle_actors[arg2->actorId]->unk3C;
    if (((D_800F19CC->unk8.unk20 & 3) == 1) || ((D_800F19CC->unk8.unk20 & 3) == 3)) {
...
void func_80085008(_hitEntity_t* arg0)
{
    int i;
    int var_v1;
    short var_a1;
    ...
    vs_skill_t* skill, _hitEntity_t* arg1, _hitEntity_t* arg2, int arg3, int hit)
{
    ...
    if ((arg3 != 0) && (arg1->unk40 == 0) && (arg1->actorId == 0) && (arg2->unk40 == 0)
        && (vs_battle_actors[arg2->actorId]->unk3C->unk957 != 0x80)
        && (skill->type == skillTypeAbility || skill->type == skillTypeBreakArt
            || skill->type == skillTypeBaseAttack)
        && (vs_battle_actors[arg2->actorId]->unk3C->currentHP <= arg2->unk4)
        && (arg2->actorId != 0) && (vs_battle_actors[arg2->actorId]->unk29 == 0)) {
        int temp_s0;
        vs_battle_actors[arg2->actorId]->unk29 = 1;
...
void func_80085718(_hitEntity_t* arg0)
{
...
void func_800859B4(u_int arg0, vs_battle_actor2* arg1, int arg2)
{
    ...
    if ((arg2 != 0) && ((arg0 - 0x16) >= 0x20)) {
        if (new_var->flags_0 == 0xFF) {
            arg1->unk954 += (arg1->weapon.unk10B + arg1->unk956_2) << 8;
        } else {
            int new_var2 = new_var->flags_0 + arg1->unk956_2;
            arg1->unk954 += new_var2 << 8;
        }
    }
}
...
void func_80089DC0(int arg0)
{
    ...
    while (_zoneContext.mpdCdFile->state != vs_main_CdQueueStateLoaded) {
        vs_main_gametimeUpdate(0);
    }
    ...
    _loadMpdRoomSection(0, (void*)(v0 + (int)mpdHeader));
    ...
    if (_getMapBounds(0) != NULL) {
        _zoneContext.bounds = *_getMapBounds(0);
    } else {
    ...
    if (((_mpdHeader*)mpdHeader)->lootSectionLen != 0) {
        ...
        _loadMpdLootSection((void*)(v0 + (int)mpdHeader));
    }

    {
        ...
        new_var = 1;
        temp_s4 = _loadMpdClearedScriptDoorSections((void*)(arg0),
            ((_mpdHeader*)mpdHeader)->clearedSectionLen, (void*)(arg2),
            ((_mpdHeader*)mpdHeader)->scriptSectionLen, (void*)(arg4), arg5,
            vs_main_startState == new_var);
    }
...
void func_8008A6FC(void)
{
    vs_battle_lootListNode* var_a0 = _lootListHead;

    while (var_a0 != 0) {
        vs_battle_lootListNode* temp_s0 = var_a0->next;
